...
void futhark_context_config_set_cuEventCreate(struct futhark_context_config *cfg, void *ptr) {
  cfg->cuEventCreate = ptr;
}

void futhark_context_config_set_cuEventDestroy(struct futhark_context_config *cfg, void *ptr) {
  cfg->cuEventDestroy = ptr;
}

void futhark_context_config_set_cuEventRecord(struct futhark_context_config *cfg, void *ptr) {
  cfg->cuEventRecord = ptr;
}

void futhark_context_config_set_cuEventElapsedTime(struct futhark_context_config *cfg, void *ptr) {
  cfg->cuEventElapsedTime = ptr;
}
...
void futhark_context_config_set_cuFuncSetAttribute(struct futhark_context_config *cfg, void *ptr) {
  cfg->cuFuncSetAttribute = ptr;
}
...
void futhark_context_set_max_block_size(struct futhark_context* ctx, size_t val) {
  ctx->max_group_size = val;
}

void futhark_context_set_max_group_size(struct futhark_context* ctx, size_t val) {
  ctx->max_group_size = val;
}
...
static void gpu_create_kernel(struct futhark_context *ctx,
  ...
  CUDA_SUCCEED_FATAL((ctx->cfg->cuModuleGetFunction)(kernel, ctx->module, name));
  // Unless the below is set, the kernel is limited to 48KiB of memory.
  CUDA_SUCCEED_FATAL((ctx->cfg->cuFuncSetAttribute)(*kernel,
...
static int gpu_launch_kernel(struct futhark_context* ctx,
  ...
  if (ctx->cfg->tracing) {
    printf("TRACE: rts: launch: kernel=\"%s\" grid=[%u, %u, %u] block=[%u, %u, %u] localmem=%u\n",
            name,
